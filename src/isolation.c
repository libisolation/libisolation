...
#define PAGE_ALIGN_MASK (PAGE_4KB - 1)
  ...
  for (int i = 0; i < ehdr->e_phnum; i++) {
    ...
    vmm_memory_map(mem, vaddr, size, prot);
  ...
  // vmm_write_register(VMM_X64_RSP, STACK_TOP);
  // vmm_write_register(VMM_X64_RBP, STACK_TOP);

  return 0;
}

static int
do_load(const char *elf_path)
{
  int fd;
  ...
  int err;
  if ((err = load_elf((Elf64_Ehdr *) data)) < 0) {
    printf("huee");
    return err;
  }
...
isl_open(const char *filename)
{
  ...
  if (do_load(filename) < 0)
    return -1;

  vmm_cpu_write_register(VMM_X64_RAX, 0);
  vmm_cpu_write_register(VMM_X64_RBX, 0);
  vmm_cpu_write_register(VMM_X64_RCX, 0);
  vmm_cpu_write_register(VMM_X64_RDX, 0);
  vmm_cpu_write_register(VMM_X64_RSI, 0);
  vmm_cpu_write_register(VMM_X64_RDI, 0);
  vmm_cpu_write_register(VMM_X64_R8, 0);
  vmm_cpu_write_register(VMM_X64_R9, 0);
  vmm_cpu_write_register(VMM_X64_R10, 0);
  vmm_cpu_write_register(VMM_X64_R11, 0);
  vmm_cpu_write_register(VMM_X64_R12, 0);
  vmm_cpu_write_register(VMM_X64_R13, 0);
  vmm_cpu_write_register(VMM_X64_R14, 0);
  vmm_cpu_write_register(VMM_X64_R15, 0);

  vmm_cpu_write_register(VMM_X64_FS, 0);
  vmm_cpu_write_register(VMM_X64_ES, 0);
  vmm_cpu_write_register(VMM_X64_GS, 0);
  vmm_cpu_write_register(VMM_X64_DS, 0);
//  vmm_cpu_write_register(VMM_X64_CS, GSEL(SEG_CODE, 0));
//  vmm_cpu_write_register(VMM_X64_DS, GSEL(SEG_DATA, 0));

//  vmm_cpu_write_register(VMM_X64_FS_BASE, 0);
//  vmm_cpu_write_register(VMM_X64_GS_BASE, 0);

  vmm_cpu_write_register(VMM_X64_LDTR, 0);
